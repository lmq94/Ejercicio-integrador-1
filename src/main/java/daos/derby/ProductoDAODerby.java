...
public class ProductoDAODerby implements ProductoDAO {
    ...
    private static ProductoDAODerby instancia;

    private ProductoDAODerby(Connection conexion) {
        this.conexion = conexion;
    }

    public static ProductoDAODerby getInstancia(Connection conexion) {
        if (instancia == null) {
            instancia = new ProductoDAODerby(conexion);
        }
        return instancia;
    }
    ...
    public void insertar(Producto elemento) throws Exception {
        ...
        try (PreparedStatement ps = conexion.prepareStatement(query)) {
            ...
            ps.setFloat(2, elemento.getValor());
            ps.executeUpdate();
